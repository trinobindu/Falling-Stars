    ...
    readJsonBody(req, (err, data) => {
      ...
      if (!newIgn || newIgn.length < 2 || newIgn.length > 16) {
        return sendJson(res, 400, {
          error: 'In-Game Name must be between 2 and 16 characters.'
        });
      }
      ...
      // If new IGN is identical to current IGN, return current profile directly
      ...
      // Check if taken by another registered player (case-insensitive)
      const existingPlayers = loadAllPlayers();
      ...
      try {
        ...
        console.log(`[IGN Change] Player "${email}" changed name from "${oldIgn}" to "${newIgn}"`);
        ...
        console.error('Error saving updated player name:', saveErr);
